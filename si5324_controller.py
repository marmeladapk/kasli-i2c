...
if __name__ == '__main__':
	mask = 0x10  # in, in,in  Out, In, In, out, out
	gpio = GpioController()
	gpio.open_from_url('ftdi://ftdi:4232h/1', mask)
	gpio.write_port(0xFF & mask)
	# gpio.close()

	port = I2cController()
	port.configure('ftdi://ftdi:4232h/1')
	switch1 = port.get_port(0x70)
	while True:
		try:
			switch1.write([0x00])
		except I2cNackError:
			pass
	switch2 = port.get_port(0x71)
	switch2.write(0x08)
	si5324 = port.get_port(0x68)
	si = Si5324(si5324)
	...
	si.set_clock_input_frequency(30, 1)
	si.set_clock_multiplication(201, 1, 1)

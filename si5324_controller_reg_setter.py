...
class Si5324_controller_reg_setter:
	"""Si5324 controller register setter class.

	This is a subclass with methods that abstract all settings of Si5324 chip that can be set in registers.

	Trying to write any reserved values will generate an exception. Trying to write values greater than register size allows will generate an exception from BitArray class.

	All methods here set register in local variables, writing these values to the device must be initiated from main class.
	"""
	...
	def set_n1_hs(self, value):
		...
		111: N1= 11
		"""
		self.regs[25][0:3] = value

	def set_nc1_ls(self, value):
		...
		"""
		# if value > (1 << 20):
		# 	raise NameError("Value %d is reserved (maximum 2^20" % value)
		value = BitArray(uint=value, length=20)
		if value[19] != 1 and value != 0:
			raise NameError("Value %d is incorrect, must be an odd number (or 0)" % value.uint)
		self.regs[31][4:8] = value[0:4]
		self.regs[32] = value[4:12]
		self.regs[33] = value[12:20]

	def set_nc2_ls(self, value):
		...
		# 	raise NameError("Value %d is reserved (maximum 2^20" % value)
		value = BitArray(uint=value, length=20)
		if (value[19] != 1) and (value.int != 0):
			raise NameError("Value %d is incorrect, must be an odd number (or 0)" % value.uint)
		self.regs[34][4:8] = value[0:4]
		self.regs[35] = value[4:12]
		self.regs[36] = value[12:20]

	def set_n2_hs(self, value):
		...
		111: 11
		"""
		self.regs[40][0:3] = value

	def set_n2_ls(self, value):
		...
		value = BitArray(uint=value, length=20)
		if value[19] != 1:
			raise NameError("Value %d is incorrect, must be an odd number" % value.uint)
		self.regs[40][4:8] = value[0:4]
		self.regs[41] = value[4:12]
		self.regs[42] = value[12:20]

	def set_n31(self, value):
		...
		"""
		value = BitArray(uint=value, length=19)
		self.regs[43][5:8] = value[0:3]
		self.regs[44] = value[3:11]
		self.regs[45] = value[11:19]

	def set_n32(self, value):
		...
		"""
		value = BitArray(uint=value, length=19)
		self.regs[46][5:8] = value[0:3]
		self.regs[47] = value[3:11]
		self.regs[48] = value[11:19]
